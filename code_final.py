...
class InterleavedLinear(nn.Module):
    def __init__(self, in_features, out_features, stride=2, bias=True):
        super().__init__()
        self.stride = stride
        self.in_features = in_features
        self.out_features = out_features
        self.select_weight = nn.Parameter(torch.randn(in_features))  # 通道重要性权重
        ...
        # 🔥 关键：用每个输入通道的 L2 norm 初始化 select_weight
        with torch.no_grad():
            self.select_weight.copy_(self.weight.norm(dim=0))  # [in_features]
    
    def forward(self, x):
        importance = self.select_weight.sigmoid()  # 通道 mask
        x = x * importance  # soft-gate input
...
class Expert(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = InterleavedLinear(in_features, hidden_features)
        self.act = act_layer()
        self.fc2 = InterleavedLinear(hidden_features, out_features)
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., num_experts=4, use_moe=False):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        ...
        if not use_moe:
            ...
            self.gate = InterleavedLinear(in_features, num_experts)
            self.drop = nn.Dropout(drop)
    ...
    def forward(self, x):
        if not self.use_moe:
            ...
            for i, expert in enumerate(self.experts):
                ...
                output += gate_weights[:, :, i].unsqueeze(-1) * expert_output
...
class Attention(nn.Module):
    def __init__(
        ...
        self.qkv = InterleavedLinear(dim, all_head_dim * 3, bias=qkv_bias)
        ...
        self.proj = InterleavedLinear(all_head_dim, dim)
...
#            self.shift_size = 0
        ...
        self.qkv = InterleavedLinear(dim, out_dim * 3, bias=qkv_bias)

        #self.qkv = nn.Conv2d(dim, out_dim * 3, 1, bias=qkv_bias)
        # self.kv = nn.Conv2d(dim, dim*2, 1, bias=False)

        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = InterleavedLinear(out_dim, out_dim)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 window_size=None, attn_head_dim=None, window=False, restart_regression=True, use_moe=False, num_experts=4):
        super().__init__()
...
class HybridEmbed(nn.Module):
    ...
    def __init__(self, backbone, img_size=224, feature_size=None, in_chans=3, embed_dim=768):
        ...
        self.proj = InterleavedLinear(feature_dim, embed_dim)
...
class RVSA_MTP_branches_Lite(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 out_indices=[11], interval=3, pretrained=None, restart_regression=True, use_moe=False, num_experts=4):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                init_values=init_values, window_size=(7, 7) if ((i + 1) % interval != 0) else self.patch_embed.patch_shape, 
                window=((i + 1) % interval != 0), restart_regression=restart_regression,
                use_moe=use_moe, num_experts=num_experts)
    ...
    def init_weights(self):
        ...
        if isinstance(pretrained, str):
            ...
            if 'state_dict' in checkpoint:
                ...
                state_dict = convert_dense_to_interleaved(state_dict, self, default_stride=2)
            else:
